...
import { FIRST_PAGE_REVIEW_COMMENTS } from "./first-page-review-comments";
...
for (const product of products) {
  const comments = FIRST_PAGE_REVIEW_COMMENTS[product.id];

  if (!comments || comments.length !== 10 || product.reviews.length < 10) {
    throw new Error(`Os 10 feedbacks iniciais de ${product.name} não foram configurados.`);
  }
